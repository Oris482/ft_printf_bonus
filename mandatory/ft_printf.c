...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/10 21:43:34 by jaesjeon          #+#    #+#             */
/*   Updated: 2022/03/10 21:43:38 by jaesjeon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_pctlst **node)
{
	...
	*node = ft_lstnew_before_str(before_str);
	if (!(*node))
		return (ERROR);
	analysis_pct(cur, &fwp);
	(*node)->after_pct = make_pct(copy_cur, &fwp, bring_arg(ap, fwp.bits), cnt);
	if (fwp.full_len != 0)
		(*node)->full_len = fwp.full_len;
	if (!(*node)->after_pct)
		return (ERROR);
	if (**cur)
		(*cur)++;
